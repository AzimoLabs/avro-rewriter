...
public class GenericData {
  ...
  private Object deepCopyRaw(Schema schema, Object value) {
    ...
    switch (schema.getType()) {
      case ARRAY:
        ...
        List<Object> arrayCopy = new Array<Object>(
      ...
      case RECORD:
        ...
        for (Field f : schema.getFields()) {
          ...
          setField(newRecord, name, pos, convertObjects(f.schema(), newValue), newState);
        }
        return newRecord;
      case STRING:
  ...
  private Object convertObjects(Schema targetSchema, Object orgValue) {
    if (orgValue == null) {
      return null;
    }

    Object result = orgValue;
    switch (targetSchema.getType()) {
      case STRING:
      case UNION:
        if (STRING_TYPE_STRING.equals(targetSchema.getProp(STRING_PROP))) {
          if (!(orgValue instanceof String)) {
            result = orgValue.toString();
          }
        } else {
          if (orgValue instanceof Utf8) {
            result = orgValue.toString();
          }
        }
      default:
        // TODO: add support for logic types?
    }
    return result;
  }
